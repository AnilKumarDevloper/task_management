...
class DashboardController extends Controller{
    public function Dashboard(){
        $recent_tasks = Task::with(['getEmployee:id,name', 'getClient.getCompanyDetail', 'getAssignedBy:id,name', 'getAmendedBy:id,name'])->where('status', 1); 
        $all_task_count = Task::where('status', 1);
        $pending_task_count = Task::where('status', 1)->where('current_status', 'pending');
        $inprocess_task_count = Task::where('status', 1)->where('current_status', 'inprocess');
        ...
        if(Auth::user()->role_id == 3){
            ...
            $pending_task_count =  $pending_task_count->where('assigned_to', Auth::user()->id);
        ...
        if(Auth::user()->role_id == 4){
            ...
            $pending_task_count =  $pending_task_count->where('client_id', Auth::user()->id);
        ...
        $pending_task_count = $pending_task_count->count();
        ...
        return view('backend.dashboard.super_admin_dashboard', compact('all_task_count', 'pending_task_count', 
