...
<body>
    <div id="main-wrapper" class="">
        <header class="topbar">
            <nav class="navbar top-navbar navbar-expand-md navbar-dark">
                <div class="navbar-header">
                    ...
                    <a class="navbar-brand" href="{{route('backend.dashboard.view')}}">
                        <b class="logo-icon">
                            ...
                            <span>
                                <img src="{{url($layout_setting->logo_image_path . '/' . $layout_setting->logo_image)}}"
                                    style="width: 40px;">
                            </span>
                        ...
                        <span class="logo-text">
                            <span class="d-flex121">
                                <spam class="logo_text_color" style="color:{{$layout_setting->logo_text_color}}">
                                    {{$layout_setting->logo_text ?? 'NDM Advisors LLP'}}</spam>
                                <span class="sub_logo_text pt-1"
                                    style="font-size:12px; color:{{$layout_setting->logo_sub_heading_color}}">
                ...
                <div class="navbar-collapse collapse header_background" id="navbarSupportedContent"
                    style="background:{{$layout_setting->navbar_color}};">
    ...
    if (Auth::user()->role_id == 4) {
        $company = App\Models\Backend\CompanyDetail::where('user_id', Auth::user()->id)->first();
                    ...
                    <ul class="navbar-nav"> 
                            ...
                            <a class="nav-link dropdown-toggle d-flex align-items-center text-white clicknottification" href="#"
                                ...
                                <div class="notify" style="left:-5px; top:-15px">
                                    ...
                                    <div class="border-radius text-white d-flex align-items-center justify-content-center "
                                        style="width:20px; height:20px; border-radius:50%;font-size:12px; background:red;">
                                        ...
                                        <div class="message-center message-body position-relative ps-container ps-theme-default"
                                            ...
                                            @if(count($notifications) > 0)
                                                @foreach($notifications as $notification)
                                                    <a href="{{route('backend.notification.view', [$notification->id])}}"
                                                        class="message-item px-2 d-flex align-items-center border-bottom py-3 mb-2  {{$notification->status == 1 ? 'unreaded_notifications' : ""}}">
                                                        ...
                                                        <div class="w-75 d-inline-block v-middle ps-3 ms-1">
                                                            ...
                                                            <span
                                                                class="fs-3 text-nowrap d-block time text-truncate fw-normal mt-1 text-muted">{{ $notification->created_at->diffForHumans() }}</span>
                                                        </div>
                                                    </a>
                                                @endforeach
                                            @endif
                        ...
                        <li class="nav-item dropdown profile-dropdown">
                            <a class="nav-link dropdown-toggle d-flex align-items-center clickprofile" href="#"
                                data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                @if(Auth::user()->profile != '')
                                    <img src="{{url(Auth::user()->profile)}}" alt="user" class="profile-pic rounded-circle"
                                        id="output" style="width:30px; height:30px; border-radius:50%;" />
                                @else
                                    <img src="{{url('assets/backend/images/users/user.png')}}" alt="user"
                            ...
                            <div
                                class="dropdown-menu dropdown-menu-end mailbox dropdown-menu-animate-up profilepageelement profilepageelementPage">
                                <ul class="list-style-none">
                                    <li class="p-30 pb-2">
                                        ...
                                        <div class="d-flex align-items-center mt-4 pt-3 pb-4 border-bottom">
                                            <div class="profile-pic">
                                                ...
                                                @if(Auth::user()->profile != '')
                                                    <img src="{{url(Auth::user()->profile)}}" id="output" width="200" />
                                                @else
                                                    <img src="{{url('assets/backend/images/users/user.png')}}" id="output"
                                                        width="200" />
                                                @endif
                                            </div>
        ...
        <aside class="left-sidebar sidbar_background_color" style="background-color:{{$layout_setting->sidebar_color}}">
            <div class="scroll-sidebar" style="height: 100%;">
                <nav class="sidebar-nav" style="height: 100%;">
                    <ul id="sidebarnav" class="sidbar_background_color"
                        style="background-color:{{$layout_setting->sidebar_color}}; height:100%">
        ...
        <div class="page-wrapper" style="background-image: url({{url($layout_setting->bg_image_path . '/' . $layout_setting->bg_image)}});
